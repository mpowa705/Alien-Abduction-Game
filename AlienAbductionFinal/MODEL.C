...
Details: Collisions and move requests mostly handled by Brady,
initializegame, nextframe, firebeam, stopfiring handled by Matt.
...
/*
Function Name: updateplanespeed
...
Purpose:
This function is called when a beam is being fired from the UFO in order to stop the beam's sound effect and
clear the beam from the screen. Then, sets the beamfiring boolean to be false.
